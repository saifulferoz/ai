...
final class ExecutionCheckpointSerializer
{
    ...
    public function fromArray(array $data): ExecutionCheckpoint
    {
        ...
        $expiresAt = isset($data['expiresAt']) && null !== $data['expiresAt'] ? new \DateTimeImmutable($data['expiresAt']) : null;
        ...
        if ($message instanceof UserMessage) {
            ...
            foreach ($message->getContent() as $part) {
                if ($part instanceof Text) {
                    ...
                    $parts[] = ['type' => 'image', 'data' => base64_encode($part->getData()), 'mimeType' => $part->getMimeType()];
                } elseif ($part instanceof Audio) {
                    $parts[] = ['type' => 'audio', 'data' => base64_encode($part->getData()), 'format' => $part->getFormat()];
                } elseif ($part instanceof Document) {
                    $parts[] = ['type' => 'document', 'data' => base64_encode($part->getData()), 'mimeType' => $part->getMimeType()];
                }
            }
    ...
    private function deserializeUserMessage(array $data): UserMessage
    {
        ...
        foreach ($data['parts'] ?? [] as $part) {
            ...
            if ('text' === $partType) {
                ...
                $contents[] = Image::fromData(base64_decode($part['data']), $part['mimeType'] ?? 'image/png');
            } elseif ('audio' === $partType && isset($part['data'])) {
                $contents[] = Audio::fromData(base64_decode($part['data']), $part['format'] ?? 'audio/mp3');
            } elseif ('document' === $partType && isset($part['data'])) {
                $contents[] = Document::fromData(base64_decode($part['data']), $part['mimeType'] ?? 'application/pdf');
            }
        }
